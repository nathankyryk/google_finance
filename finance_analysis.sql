...
SELECT ci.industry, (AVG(sd.percent_change) * 100)::NUMERIC(10, 2) AS avg_percent_increase
FROM stockdata SD
...
SELECT sd.company_name, (sd.percent_change * 100)::NUMERIC(10,0) AS percent_increase, ci.industry
FROM stockdata SD 
JOIN companyinfo CI ON sd.ticker = ci.ticker
ORDER BY sd.percent_change DESC;

-- Companies With the Worst Return
SELECT sd.company_name, sd.percent_change * 100 AS percent_increase, ci.industry
FROM stockdata SD 
JOIN companyinfo CI ON sd.ticker = ci.ticker
...
SELECT SD.company_name, CI.industry, 
	  (sd.percent_change * 100)::NUMERIC(10, 2) AS company_return,
	  (AVG(SD.percent_change) OVER (PARTITION BY ci.industry) * 100)::NUMERIC(10,2) AS industry_avg,
	  ((sd.percent_change * 100) - (AVG(sd.percent_change) OVER (PARTITION BY ci.industry) * 100))::NUMERIC(10, 2) AS performance_vs_industry_avg
FROM stockdata SD
JOIN companyinfo CI ON sd.ticker = ci.ticker
